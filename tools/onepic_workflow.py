...
def review_character_candidate() -> int:
    ...
    dialog = QDialog()
    dialog.setWindowTitle("确认标准角色 · 小u桌面角色")
...
def create_walk_review(manifest_path: Path) -> dict[str, Any]:
    ...
    if not manifest_file.is_file():
        raise FileNotFoundError(f"找不到私有角色清单：{manifest_file}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="管理一图桌面角色的人工确认门禁")
    ...
    commands.add_parser("check-package", help="检查是否允许加载或打包私有桌面角色")
