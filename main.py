"""
本模块是 Windows 与 macOS 桌面角色应用的命令行启动入口。

职责范围：
- 解析可选的 `--smoke-test-ms` 自动退出参数；
- 调用 xiaou_desktop.app.run() 启动 Qt 应用；
- 不包含窗口、行为或配置业务逻辑。

Agent 快速定位：
- 命令行参数在 parse_args() 中定义；
- 主入口位于 main()；
- 具体应用生命周期请阅读 src/xiaou_desktop/app.py。
...
from xiaou_desktop.app import run


def parse_args() -> argparse.Namespace:
    """解析启动参数。"""

    parser = argparse.ArgumentParser(description="启动 Windows 桌面角色")
    parser.add_argument(
        "--smoke-test-ms",
        type=int,
        default=None,
