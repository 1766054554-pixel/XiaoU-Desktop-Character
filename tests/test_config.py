"""
本模块测试桌面角色配置的合并、范围校验、损坏回退和原子保存行为。
...
from xiaou_desktop.config import PetSettings, load_settings, save_settings
...
def test_load_settings_merges_position_and_user_selected_size(tmp_path) -> None:
    ...
    assert not settings.speech_enabled
...
def test_animation_timing_is_limited_to_safe_ranges(tmp_path) -> None:
    ...
    assert settings.mood_update_interval_ms == 30000
