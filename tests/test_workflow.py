"""
本模块测试一图桌面角色的原图登记、标准角色确认、走路确认门禁和个人包隐私边界。
...
from xiaou_desktop.workflow import (
    WorkflowError,
...
def test_personal_build_only_bundles_runtime_private_assets() -> None:
    """本地定制包不得把原图、备份和图集草稿所在的整个私有目录打进去。"""

    for spec_name in (
        "XiaoUDesktopCharacter-Windows.spec",
        "XiaoUDesktopCharacter-Mac.spec",
    ):
        spec = (PROJECT_ROOT / spec_name).read_text(encoding="utf-8")

        assert 'datas.append(("user_assets", "user_assets"))' not in spec
        assert 'private_assets / "pet"' in spec
        assert 'private_assets / "selfie.png"' in spec
        assert 'private_assets / "workflow.json"' in spec
        assert 'os.environ.get("XIAOU_INCLUDE_USER_ASSETS")' in spec
        assert "ONEPIC_INCLUDE_USER_ASSETS" not in spec
